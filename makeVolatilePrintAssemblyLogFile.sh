...
export compilation="-server -Xcomp -XX:+LogCompilation -XX:LogFile=volatile.log"
...
echo "$JAVA_HOME/bin/java" $REQUIRED_SWITCHES $OPTIONAL_SWITCHES -cp "$CLASSPATH" PrintAssemblyVolatile

"$JAVA_HOME/bin/java" $REQUIRED_SWITCHES $OPTIONAL_SWITCHES -cp "$CLASSPATH" PrintAssemblyVolatile 2>&1 >/dev/null
