...
export compilation="-server -Xcomp -XX:+LogCompilation -XX:LogFile=synchronized.log"
...
echo "$JAVA_HOME/bin/java" $REQUIRED_SWITCHES $OPTIONAL_SWITCHES -cp "$CLASSPATH" PrintAssemblySynchronized

"$JAVA_HOME/bin/java" $REQUIRED_SWITCHES $OPTIONAL_SWITCHES -cp "$CLASSPATH" PrintAssemblySynchronized 2>&1 >/dev/null
